...
dfFileName <- data.frame(c("sync_data_2018-07-13_cal.csv",
                           "sync_data_2018-07-18_cal.csv",
                           "sync_data_2018-07-20_cal.csv",
                           "sync_data_2018-07-24_cal.csv",
                           "sync_data_2018-07-27_cal.csv",
                           "sync_data_2018-08-03_cal.csv",
                           "sync_data_2018-08-09_cal.csv",
                           "sync_data_2018-08-10_cal.csv",
                           "sync_data_2018-08-15_cal.csv",
                           "sync_data_2018-08-23_cal.csv",
                           "sync_data_2018-09-05_cal.csv",
                           "sync_data_2018-09-13_cal.csv",
                           "sync_data_2018-10-05_cal.csv",
                           "sync_data_2018-10-12_cal.csv",
                           "sync_data_2018-10-16_cal.csv",
                           "sync_data_2018-10-24_cal.csv",
                           "sync_data_2018-10-26_cal.csv"))

# Initiating lists required to store matrix of data for each file
GroupEnhancedPeak <- list()
GroupEnhanced <- list()
GroupMPlot <- list()
GroupLargePlot <- list()
GroupWFreq <- list()
GroupWRose <- list()
GroupDataIDs <- list()
GroupConeCentroid <- list()

# Number of unique colours needed (Must be greater than number of files)
nColor <- 60

# Iterates for each file
for (f in 1:nrow(dfFileName)) {

  #Select File (Change Location as Needed but keep file naming convention the same)
  filename <- paste("/Data_Directory/",toString(dfFileName[f,1]), sep = "")
  ...
  # Create dataset of only red dots
  ...
  # Secondary Source [rotating quadrant]
  ...
  #Finding Centroid of 5 min running average cone to cleanly identify areas of unknown sources
  #X Coordinate for 5min running average Cone
  dfEnhanced$X1 <- cos(dfEnhanced$Lat1Rad)*cos(dfEnhanced$Lon1Rad)
  dfEnhanced$X5A <- cos(dfEnhanced$Lat5ARad)*cos(dfEnhanced$Lon5ARad)
  dfEnhanced$X5B <- cos(dfEnhanced$Lat5BRad)*cos(dfEnhanced$Lon5BRad)
  
  #Y Coordinate for 5min running average
  dfEnhanced$Y1 <- cos(dfEnhanced$Lat1Rad)*sin(dfEnhanced$Lon1Rad)
  dfEnhanced$Y5A <- cos(dfEnhanced$Lat5ARad)*sin(dfEnhanced$Lon5ARad)
  dfEnhanced$Y5B <- cos(dfEnhanced$Lat5BRad)*sin(dfEnhanced$Lon5BRad)
  
  #Z Coordinate for 5min running average
  dfEnhanced$Z1 <- sin(dfEnhanced$Lat1Rad)
  dfEnhanced$Z5A <- sin(dfEnhanced$Lat5ARad)
  dfEnhanced$Z5B <- sin(dfEnhanced$Lat5BRad)
  
  #Centroid X,Y,Z Coordinate for 5min running average
  dfEnhanced$XCentroid <- dfEnhanced$X1 + dfEnhanced$X5A + dfEnhanced$X5B
  dfEnhanced$YCentroid <- dfEnhanced$Y1 + dfEnhanced$Y5A + dfEnhanced$Y5B
  dfEnhanced$ZCentroid <- dfEnhanced$Z1 + dfEnhanced$Z5A + dfEnhanced$Z5B
  
  #Radian Centroid Coordinate for 5min running average
  dfEnhanced$LonCentroidRad <- atan2(dfEnhanced$YCentroid, dfEnhanced$XCentroid)
  dfEnhanced$HypCentroidRad <- sqrt(dfEnhanced$XCentroid^2 + dfEnhanced$YCentroid^2)
  dfEnhanced$LatCentroidRad <- atan2(dfEnhanced$ZCentroid, dfEnhanced$HypCentroidRad)
  
  #Lon/Lat Centroid Coordinate for 5min running average
  dfEnhanced$LatCentroidDeg <- NISTradianTOdeg(dfEnhanced$LatCentroidRad)
  dfEnhanced$LonCentroidDeg <- NISTradianTOdeg(dfEnhanced$LonCentroidRad)
  ...
  RemovePeaksList <- apply(distMatRemovePeaks, 1,function(x) which(x <1500 & x >0))
  ...
  GroupEnhancedPeak[[f]] <- dfEnhancedPeak
  GroupEnhanced[[f]] <- dfEnhanced
  GroupMPlot[[f]] <- MPlot
  GroupLargePlot[[f]] <- LPlot
  GroupWFreq[[f]] <- wFreq
  GroupWRose[[f]] <- wRose
  GroupDataIDs[[f]] <- dataID
  
  #Writing the data to CSV and saving the file (change location as needed)
  tablePeak <- paste('/Results_Directory/', dataID ,'-PeakAndWind.csv', sep="")
  write.csv(dfEnhancedPeak, file = tablePeak, row.names = F)
}

# Creating PDFs for each dataset
for (k in 1:length(GroupEnhancedPeak)) {
  plotLocF <- paste('/Results_Directory/', toString(GroupDataIDs[[k]]) ,'-PeakAndWind.pdf', sep="")
  pdf(plotLocF)
  for (i in 1:length(MPlot)) {
    plot(GroupMPlot[[k]][[i]])
  }
  plot(GroupLargePlot[[k]])
  plot(GroupWFreq[[k]])
  plot(GroupWRose[[k]])
  dev.off()
}
...
# Uncomment below if map is not refreshing properly
# clearShapes(MapEnhance)

# Map
MapEnhance <- leaflet(GroupEnhancedPeak[[1]]) %>%
  addTiles('http://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}.png',
           attribution=paste('Ashbridges Bay',' Map tiles by <a href="http://stamen.com">Stamen Design</a>, <a href="http://creativecommons.org/licenses/by/3.0">CC BY 3.0</a> &mdash; Map data &copy; <a href="http://www.openstreetmap.org/copyright">OpenStreetMap</a>')) %>%
  setView(GroupEnhancedPeak[[1]]$lon[1],GroupEnhancedPeak[[1]]$lat[1], zoom = 13 )
for (k in 1:length(GroupEnhancedPeak)) {
  # Labels 
  labs <- lapply(seq(nrow(GroupEnhanced[[k]])), function(i) {
    paste0( 'Time (UTC): ', GroupEnhanced[[k]][i, "gps_time"], '<p></p>',
            'CH4 (ppm): ', GroupEnhanced[[k]][i, "ch4"], '<p></p>',
            'Wind Speed (m/s): ', GroupEnhanced[[k]][i, "ws_corr"],'</p><p>',
            'Wind Direction (degrees): ', GroupEnhanced[[k]][i, "wd_corr"], '<p></p>',
            'Enhancement Measured (ppm): ', GroupEnhanced[[k]][i, "enhance0.05"], '<p></p>',
            'Closest Source: ',GroupEnhanced[[k]][i, "TP"], '<p></p>',
            'Dist to Closest Source (m): ', GroupEnhanced[[k]][i, "Clos"],'</p><p>',
            'SourceID: ', GroupEnhanced[[k]][i, "SourceID"], '<p></p>',
            'Secondary Closest Source: ',GroupEnhanced[[k]][i, "SecTP"], '<p></p>',
            'Dist to Secondary Closest Source (m): ', GroupEnhanced[[k]][i, "SecClos"],'</p><p>',
            'Secondary SourceID: ', GroupEnhanced[[k]][i, "SecSourceID"], '<p>')
  })

  MapEnhance <- MapEnhance %>%
    # Adding only unknown enhancements (change ifelse statement to see all enhancements)
    addCircles(GroupEnhancedPeak[[k]]$lon, GroupEnhancedPeak[[k]]$lat, popup=GroupEnhancedPeak[[k]]$TP, label= ~lapply(labs, HTML), weight = 1, radius=100,
    color=~ifelse(GroupEnhancedPeak[[k]]$SourceID == "N/A", col_vector[k],"transparent"), stroke = TRUE, fillOpacity = 0.5) %>%
    # Facility Markers
    addMarkers(~dfFacility$Longitude, ~dfFacility$Latitude, popup = ~dfFacility$TreatmentPlant, label = ~dfFacility$TreatmentPlant)
  
  # Adding Cones for unknown enhancements (comment to not see cones + centroids)
  for (j in 1:nrow(GroupEnhancedPeak[[k]])) {
    if (GroupEnhancedPeak[[k]]$SourceID[j] == "N/A") {
      MapEnhance <- MapEnhance %>%
        addPolygons(lat = as.numeric(GroupEnhancedPeak[[k]][j, c('Lat1deg', 'Lat5Adeg', 'Lat5Bdeg', 'Lat1deg')]),
        lng = as.numeric(as.numeric(GroupEnhancedPeak[[k]][j, c('Lon1deg', 'Lon5Adeg', 'Lon5Bdeg', 'Lon1deg')])), color = col_vector[k]) %>%
        # Adding centroid circle markers
        addCircleMarkers(~GroupEnhancedPeak[[k]][j, 'LonCentroidDeg'],~GroupEnhancedPeak[[k]][j, 'LatCentroidDeg'], radius = 10, color = col_vector[k])
  }
  }
}

#Saving Map as HTML File
MapLoc<- paste('/Results_Directory/','2018GHGMap.html', sep="")
mapshot(MapEnhance, MapLoc)
